  ...
  render() {
    ...
    Object.keys(this.props.entity.children).forEach((key, index) => {
      const item = this.props.entity.children[key],
        ...
        vaccinesAdministered = item.vaccinationData ? item.vaccinationData.totalPeopleVaccinatedOneDose[item.vaccinationData.totalPeopleVaccinatedOneDose.length - 1] : null,
    ...
    const columns = [
      ...
      { field: 'pctVaccinatedOneDose', headerName: '% Vacc. (1 Dose)', width: 150, valueFormatter: (params) => `${formatHelpers.formatPercentage(params.value, false)}${this.getRatio(params.value, 100)}`, headerClassName: styles.header, cellClassName: styles.value, type: 'number' },
      { field: 'pctVaccinatedAllDoses', headerName: '% Vacc. (All Doses)', width: 150, valueFormatter: (params) => `${formatHelpers.formatPercentage(params.value, false)}${this.getRatio(params.value, 100)}`, headerClassName: styles.header, cellClassName: styles.value, type: 'number' },
